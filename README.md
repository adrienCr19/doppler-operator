...
| N25 | **raising every index with Doppler operators only**: pure-`D` middle-index step (P3) via the factor dictionary, plus axis ladders (P0), reflection (N1), rank-one fill (P2) — no kernels, brackets, or extended `X` family. P3 is shown (harness 39) to be a **linear recurrence with explicit elementary (J-function) coefficients** for general `l'`; the `q→q−1` shift is essential and the off-axis raise is provably non-elementary. Run to conclusion, the ladder generates the **closed form of every `D_{ll'l''}(q)` up to `D555`** — no kernels — tabulated in `proofs/closed-forms-catalog.html` (harness 40, all 216 verified to dev 0). The ladder is further **solved in closed form** (harness 41): P3′ resums into a finite spectral-shift sum over the axis family with elementary weights, giving a **master formula for arbitrary `(l,l',l'')` directly from `D_{a00}`** — no stepping |
...
- `verification/` — 41 Wolfram-language harnesses + saved PASS logs (`out_*.txt`) + browsable per-script pages.
- `data/` — machine-readable exact tables:
  ...
  `closed_forms_pureD.json` (all 216 exact closed forms `D_{ll'l''}(q)`, l,l',l'' = 0…5, in `γ,p,J_{q+k}` form,
  generated by the pure-D ladder).
