...
| N25 | **raising every index with Doppler operators only**: pure-`D` middle-index step (P3) via the factor dictionary, plus axis ladders (P0), reflection (N1), rank-one fill (P2) — no kernels, brackets, or extended `X` family. P3 is shown (harness 39) to be a **linear recurrence with explicit elementary (J-function) coefficients** for general `l'`; the `q→q−1` shift is essential and the off-axis raise is provably non-elementary. Run to conclusion, the ladder generates the **closed form of every `D_{ll'l''}(q)` up to `D555`** — no kernels — tabulated in `proofs/closed-forms-catalog.html` (harness 40, all 216 verified to dev 0) |
